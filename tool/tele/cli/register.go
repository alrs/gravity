...
func RegisterCommands(app *kingpin.Application) Application {
	...
	tele.BuildCmd.Set = tele.BuildCmd.Flag("set", "Set Helm chart values on the command line. Can specify multiple or comma-separated: key1=val1,key2=val2.").Strings()
	tele.BuildCmd.Values = tele.BuildCmd.Flag("values", "Set Helm chart values from the provided YAML file(-s).").Strings()
